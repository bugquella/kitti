...
StereoMessagesSync2::StereoMessagesSync2(ros::NodeHandle nh, std::string camera1_topic_name,
                                       std::string camera2_topic_name,
                                       std::string lidar_topic_name,
...
subCamera2Image_(nh, camera2_topic_name, 20),
subLidarData_(nh,lidar_topic_name, 10),
subGpsData_(nh,gps_topic_name, 30),
subImuData_(nh,imu_topic_name, 30),
subVelData_(nh,vel_topic_name, 30),

sync(MySyncPolicy(10), subCamera1Image_, subCamera2Image_, subLidarData_, subGpsData_,subImuData_,subVelData_)
{
  ROS_INFO("lidar and stereo camera data synchronizing started");
  sync.registerCallback(boost::bind(&StereoMessagesSync2::stereocameraLidarCallback, this,_1, _2, _3,_4,_5,_6));
}

StereoMessagesSync2::~StereoMessagesSync2() {
}


void StereoMessagesSync2::stereocameraLidarCallback(const sensor_msgs::ImageConstPtr& image1_msg,
                               const sensor_msgs::ImageConstPtr& image2_msg,
                               const sensor_msgs::PointCloud2ConstPtr& lidar_msg,
                               ...
                               const geometry_msgs::TwistStampedConstPtr& vel_msg)
{
  ...
  result.image2_ptr = image2_msg;
  result.cloud_ptr = cloudMsg;
  result.gps_ptr = gps_msg;
  result.imu_ptr = imu_msg;
  result.vel_ptr = vel_msg;
  

  messages_queue_.push(result);
}

bool StereoMessagesSync2::is_valid()
{
  if (messages_queue_.empty())
    return false;
  else
...
sensors_fusion::SynchronizedMessages  StereoMessagesSync2::getSyncMessages()
{
  if(!messages_queue_.empty()) {
    sensors_fusion::SynchronizedMessages res = messages_queue_.front();
    messages_queue_.pop();
